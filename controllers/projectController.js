...
const getProject = async (req, res) => {
  ...
  const project = await Project.findById(id)
    .populate({
      path: "tasks",
      populate: { path: "completeBy", select: "nameUser" },
    })
    .populate("collaborators", "nameUser email");
  if (!project) {
    const error = new Error("Project Not Found");
    return res.status(404).json({ msg: error.message });
  }
...
const deleteProject = async (req, res) => {
  ...
  try {
    ...
    res.json({ msg: `Project ${project.nameProject} Succesfully Deleted` });
  } catch (error) {
    console.log(error);
  }
};
...
const searchCollaborator = async (req, res) => {
  ...
  const user = await User.findOne({ email }).select(
    "-password -createdAt -updatedAt -token -__v -confirmUser"
  );
  if (!user) {
...
const addCollaborator = async (req, res) => {
  ...
  await project.save();
  res.json({
    msg: `Collaborator ${user.nameUser} added to the project `,
  });
};
